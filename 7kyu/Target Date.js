...
function dateNbDays(a0, a, p) {
  const persentPerDay = p / 100 / 360;
  const startDateDeposit = new Date(2016, 0, 1).getTime();
  let count = 0;
  while (a0 < a) {
    a0 += a0 * persentPerDay;
    count++;
  ...
  const stopDateDeposit = new Date(startDateDeposit + count);
  const stopYear = stopDateDeposit.getFullYear();
  const stopMonth = stopDateDeposit.getMonth();
  const stopDay = stopDateDeposit.getDate();
  return `${stopYear}-${
    stopMonth >= 9 ? stopMonth + 1 : '0' + (stopMonth + 1)
  }-${stopDay < 10 ? '0' + stopDay : stopDay}`;
}
