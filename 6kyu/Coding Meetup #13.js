...
function isLanguageDiverse(list) {
  const langObj = list.reduce((acc, el) => {
    acc[el.language] = (acc[el.language] || 0) + 1;
    return acc;
  }, {});
  const counterLangArr = Object.values(langObj).sort((a, b) => b - a);
  const flag1 = +counterLangArr[0] / +counterLangArr[1];
  const flag2 = +counterLangArr[0] / +counterLangArr[2];
  return flag1 > 2 || flag2 > 2 ? false : true;
}
